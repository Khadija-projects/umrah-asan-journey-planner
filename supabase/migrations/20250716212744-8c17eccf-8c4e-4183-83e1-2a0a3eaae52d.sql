...
-- Get the partner profile ID first
DO $$
DECLARE
...
BEGIN
    ...
    IF guest_profile_id IS NOT NULL THEN
        ...
        LOOP
            ...
            SELECT id INTO hotel_record 
            FROM hotels 
            WHERE LOWER(name) LIKE LOWER('%' || booking_record.hotel_name || '%')
            LIMIT 1;
            
            -- If no exact match, get first hotel
            IF hotel_record IS NULL THEN
                SELECT id INTO hotel_record FROM hotels LIMIT 1;
            END IF;
            
            -- Get a room for this hotel
            SELECT id INTO room_record 
            FROM rooms 
            WHERE hotel_id = hotel_record 
            LIMIT 1;
            
            -- Insert into main bookings table
            IF hotel_record IS NOT NULL AND room_record IS NOT NULL THEN
